...
// UV座標の計算
    ...
    float u_intensity = 1.0; // 強度調整用の変数
    float u_colorShift = 400.0; // 色相調整用の変数
...
for (int i = 0; i < 5; i++) {
    ...
    float hue = (fi * 0.2 + time * 0.1 + u_colorShift) + atan(rotSt.y, rotSt.x) / (2.0 * 3.14159);
...
for (int x = -1; x <= 1; x++) {
    for (int y = -1; y <= 1; y++) {
        ...
        // 点滅効果
        sparkle += smootherstep(0.05, 0.0, dist) * (0.5 + 0.5 * sin(time * 5.0 * u_intensity + randomOffset * 10.0));
    }
}


float centerDist = length(st);
vec3 centerGlow = hsv2rgb(vec3(time * 0.05 + u_colorShift, 0.9, 1.0));

// 中央からの放射光
color += glow(centerDist, 0.8, 2.0) * centerGlow * 0.3;

// 各エフェクトを合成
color += abs(waves) * hsv2rgb(vec3(time * 0.1 + u_colorShift + 0.3, 0.7, 0.6));
color += sparkle * hsv2rgb(vec3(time * 0.15 + u_colorShift + 0.6, 0.9, 1.0)) * 0.8;
